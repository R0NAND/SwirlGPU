...
fn computeMain(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  if(global_id.x > 0 && global_id.x < grid_size.x - 1 && global_id.y > 0 && global_id.y < grid_size.y - 1) {
    ...
    densities_out[cell_index] = density - diff * dt* (4.0 * density - densities_in[left_index] - densities_in[right_index] - densities_in[top_index] - densities_in[bottom_index]);
