...
@group(2) @binding(0) var<storage> densities_in: array<f32>;
@group(2) @binding(1) var<storage, read_write> densities_out: array<f32>;
...
fn computeMain(@builtin(global_invocation_id) global_id: vec3<u32>) {
  if(global_id.x > 0 && global_id.x < grid_size.x - 1 && global_id.y > 0 && global_id.y < grid_size.y - 1) {
    ...
    let t0: f32 = 1.0 - t1;
    densities_out[index] = s0 * (t0 * densities_in[i0 + grid_size.x * j0] + t1 * densities_in[i0 + grid_size.x * j1]) + 
                           s1 * (t0 * densities_in[i1 + grid_size.x * j0] + t1 * densities_in[i1 + grid_size.x * j1]);
	}
}