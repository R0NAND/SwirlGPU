...
@group(1) @binding(0) var<storage, read_write> divergences_in: array<f32>;

@group(2) @binding(0) var<storage> pressures_in: array<f32>;
@group(2) @binding(1) var<storage, read_write> pressures_out: array<f32>;
...
fn computeMain(@builtin(global_invocation_id) global_id: vec3<u32>) {
  if(global_id.x > 0 && global_id.x < grid_size.x - 1 && global_id.y > 0 && global_id.y < grid_size.y - 1) {
    ...
    let divergence = divergences_in[cell_index];
